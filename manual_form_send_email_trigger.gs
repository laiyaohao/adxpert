function SendemailsManualForm() { //not done yet
  //get a spreadsheet and choose Record of Account Opening Forms, the sheet we are working on
  var ss = SpreadsheetApp.getActiveSpreadsheet();
  ss.setActiveSheet(ss.getSheetByName("Manual Form Record of Account Opening Forms"));
  ...
  for (var i = 0;i<sheet.getLastRow()-1;i++) { //if this code fails, replace "sheet.getLastRow()-1" with 998
    ...
    if (datecell != "" && statuscell == ""){
      ...
      //start getting the new excel sheet ready
      ...
      if (cheetahorpanda == '电商' || cheetahorpanda == '其他' || cheetahorpanda == '科技' || cheetahorpanda == 'Information') {
...
function replymailforBMtying() {
  var ss = SpreadsheetApp.getActiveSpreadsheet();
  ss.setActiveSheet(ss.getSheetByName("Send emails for BM tying Manual form"));
  var sheet = SpreadsheetApp.getActiveSheet();
